...
def build_system_prompt(today_iso: str | None = None) -> str:
    ...
    return (
        ...
        "SERVER-PARSED DATES: The Saayro server parses natural-language date inputs (ISO, dot-format, month-name, slash, duration like 'for 5 days', and relative phrases like 'next weekend' or 'in two weeks') and writes confirmed `start_date`/`end_date` into `planning_state` BEFORE you see them. If both dates are already in `planning_state`, do NOT re-ask. If `planning_state.expected_answer_type == 'dates'` and the server has surfaced candidate interpretations in `options` (e.g., '2026-05-02 to 2026-05-06'), do NOT invent dates — let the user pick or retype.\n"
